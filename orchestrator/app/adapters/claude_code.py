"""Claude Code adapter (Phase 4).

Boundary only: Claude Code is **not connected** in Phase 4.  Health reports
...
class ClaudeCodeAdapter(AgentAdapter):
    """Adapter for the ``claude_code`` provider (boundary only in Phase 4)."""
    ...
    def check_health(self, agent_id: uuid.UUID) -> AgentHealth:
        return AgentHealth(
            ...
            detail=(
                "claude_code provider is not configured: "
                "Claude Code integration lands in a later phase."
            ),
